    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                TimeSpan lagToDequeue = System.DateTime.Now - rep.GetTime();
                TimeSpan lagBetweenPackets = rep.GetTime() - ts_prev;
                DebugPrint(string.Format(
                    "Dequeue. Queue length: {0}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: {3:F1} ms. Lag between packets (expect 15ms): {4:F1} ms",
                    reports.Count, report_buf[0], report_buf[1], lagToDequeue.TotalMilliseconds, lagBetweenPackets.TotalMilliseconds
                ), DebugType.THREADING);
